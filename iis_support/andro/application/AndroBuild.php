...
class x_builder {
	...
	function DB_Connect()
	{
		...
		if ($GLOBALS["dbconna"]) {
			...
			if ($parm['APP']=='andro') {
				$GLOBALS['dir_andro']='';
				$this->LogEntry("Building Node Manager, no linking of LIB directory.");
			}
			else {
...
;';
	...
	function SP_Load() {
		...
		ob_start();
		?>
CREATE OR REPLACE FUNCTION zdd.Table_Sequencer() RETURNS void AS $BODY$
DECLARE rowcount integer := 1; lnSeq integer := 1; BEGIN UPDATE
zdd.tables_c set table_seq = 0; DELETE FROM zdd.table_deps; INSERT INTO
zdd.table_deps (table_id_par,table_id_chd) SELECT table_id_par, table_id
FROM zdd.tabfky_c WHERE zdd.tabfky_c.nocolumns <> 'Y' AND
zdd.tabfky_c.table_id <> zdd.tabfky_c.table_id_par; UPDATE zdd.tables_c
set table_seq = -1 FROM zdd.table_deps f WHERE table_id = f.table_id_chd
; while rowcount > 0 LOOP UPDATE zdd.tables_c set table_seq = lnSeq FROM
(SELECT t1.table_id_chd FROM zdd.table_deps t1 JOIN zdd.tables_c t2 ON
t1.table_id_par = t2.table_id GROUP BY t1.table_id_chd HAVING
MIN(t2.table_seq) >= 0) fins WHERE zdd.tables_c.table_id =
fins.table_id_chd AND zdd.tables_c.table_seq = -1; lnSeq := lnSeq + 1;
GET DIAGNOSTICS rowcount = ROW_COUNT; END LOOP; RETURN; END; $BODY$
LANGUAGE plpgsql;
		<?php
 ...
 ORDER BY 3,4,5";
      ...
      $dira = AddSlash($parm['DIR_LINK_APP']);
			...
			while($row=pg_fetch_array($dbres)) {
				...
				if($row['flag_vis']<>'Y') {
					$file=$this->FS_ADDSLASH($dir_pubx . $tgt) . '.htaccess';
					$text="Deny From All";
			...
			// Justin Dearing 12/26/07, removed shell execs 
			$this->fsCopyPath($dir_pubx . "root", $dir_pubx);
			
			$this->LogEntry("  Copying " . $this->FS_ADDSLASH($dir_pubx . "root") . "htaccess to " . $dir_pubx . ".htaccess");
			copy($this->FS_ADDSLASH($dir_pubx . "root") . "htaccess", $dir_pubx . ".htaccess");
			
			$this->LogEntry("  Deleting " . $dir_pubx . "htaccess");
      ...
      `$cmd`;
		...
		function FS_ADDSLASH($input) {
			// Justin Dearing 12/26/07, Function now uses \ instead of / on windows.
			$input = trim($input);
			$dir_delimeter = $this->isWindows() ? "\\" : '/';
			if (ereg('[\\/]$', $input) === FALSE) {
				$input .= $dir_delimeter;
			}
			return $input;
		...
		function LogStart()
		{
			...
			$parm["DIR_TMP"]=$this->FS_ADDSLASH($parm["DIR_PUB"]."tmp");
      ...
      .' (build total: '.$this->hCount(time()-$this->ts).' seconds)'
		...
		// Justin Dearing 12/26/2007, Added from code posted by SkyEye on php.net
		// http://www.php.net/manual/en/function.copy.php
		function fsCopyPath($srcdir, $dstdir, $recursive=false) {
			$num = 0;
		...
		function WhoAmI() {
			...
			if ($this->isWindows()) {
				...
				return ArraySafe($_SERVER, "USERNAME", "");
			} else {
